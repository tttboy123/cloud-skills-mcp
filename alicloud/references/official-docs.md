...
The universal adapter signs current general OpenAPI requests with ACS3, product APIs that still document legacy RPC or ROA V2 with HMAC-SHA1, MaxCompute project/data/Tunnel requests with ODPS V2 or V4, classic Function Compute requests with FC HMAC-SHA256, current `fcapp.run` triggers with their Action-less ACS3 variant, custom-domain triggers with the documented POP HMAC-SHA1 request, OSS data-plane requests with V1 or V4 Header authentication, SLS data-plane requests with v1 or v4, MNS requests with its service-specific HMAC-SHA1 protocol, and Tablestore protobuf data-plane requests with OTS v2 or v4. It never executes Alibaba Cloud CLI.

...
| Baidu AI Cloud | `baidubce.com`/BOS `bcebos.com` signed HTTPS，支持 `bce-auth-v1` 与按 API 选择 v2；CCR 企业版/个人版 Docker/OCI Registry；RTC AI Agent 由 BCE v1 控制面创建后用内部实例 token 建立 raw/raw16k/PCMA/PCMU/G.722/Opus 双工 WSS | BCE AK/SK、IAM/STS temporary AK/SK/session token；CCR 临时登录与 Bearer token 仅在 server 内部派生；RTC 产品 license 仅由 server 环境注入 |
...
Baidu IoT Core 的 IAM 应用权限 MQTT 3.1.1 数据面使用 `auth_scheme=iotcore-mqtt-ws`，只接受精确的 `wss://<iot-core-id>.iot.gz.baidubce.com/mqtt` 和 WebSocket `mqtt` 子协议。调用方仅声明无凭证的 client/topic/QoS/消息上限计划；server 按官网双重 HMAC-SHA256-HEX 公式从 `BCE_ACCESS_KEY_ID`/`BCE_SECRET_ACCESS_KEY` 派生 CONNECT username/password，执行 CONNECT、按官网每包 8 个的限制覆盖单连接最多 100 个订阅、SUBSCRIBE/PUBLISH、QoS 1 ACK、PING 和 DISCONNECT，派生凭证不会进入 MCP、审计或错误。消息默认上限为 32 KiB；实例经官网流程获批后可显式设置 `max_payload_bytes`，但不得超过 128 KiB，收发两侧都执行同一边界；publish 由 server 按官网 QoS 0/1 速率限制自动节流。官网应用权限格式没有 STS session-token 字段，因此此路径在配置 `BCE_SESSION_TOKEN`/`BCE_SECURITY_TOKEN` 时 fail closed。订阅是 read tool；publish、持久会话和 Will 必须走 mutation gate：

```json
{"name":"baiducloud_api_read","arguments":{"auth_scheme":"iotcore-mqtt-ws","service":"iotcore","operation":"SubscribeMQTT","method":"GET","url":"wss://<iot-core-id>.iot.gz.baidubce.com/mqtt","body":{"client_id":"observer-1","subscriptions":[{"topic_filter":"sensors/+/temperature","qos":1}],"max_messages":1,"timeout_seconds":60},"response_file":"/approved/results/baidu-iotcore.ndjson"}}
```

...
| Alibaba Cloud | ACS3；旧版 RPC/ROA V2；OSS4；SLS v1/v4；MNS；OTS v2/v4 签名 HTTPS | 官方 credentials-go：AKSK/STS、RAM/OIDC、ECS RAM Role |
...
Alibaba ACS3 使用 `auth_scheme=acs3`；仍要求旧版 RPC/ROA V2 HMAC-SHA1 的产品分别使用 `auth_scheme=rpc|roa`；OSS 使用 `oss4`，SLS 使用 `sls|sls4`，MNS 使用 `mns`，Tablestore protobuf 数据面使用 `ots|ots4`。ACS3/RPC/ROA 均使用 `api_version`、`operation` 和精确 URL。Tencent API 3.0 使用 `auth_scheme=tc3`、`api_version`、`operation` 和精确 URL；COS 使用 `auth_scheme=cos`。Baidu 使用 `auth_version=v1|v2`。六云二进制或媒体 request body 都可使用受控 `body_file`；对象下载、日志导出和备份响应使用 `response_file`，文件内容不会进入模型上下文。

...
| Tencent Cloud | API 3.0 TC3 与 v1 HmacSHA1/HmacSHA256 HTTPS；仍在运行的旧版 qcloud API 2017；COS 数据面 signed HTTPS；ASR、虚拟号真人判定、实时语音翻译、音色变换、MPS 识别/翻译与 MPS TTS signed WSS 内部流 | SecretId/SecretKey 或 CAM/STS 临时三元组；这些 WSS 接口按官方约束使用长期 SecretId/SecretKey |

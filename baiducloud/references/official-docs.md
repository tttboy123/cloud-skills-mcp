...
- IoT Core current broker connection information and WSS port 443: https://cloud.baidu.com/doc/IoTCore/s/hk7omsfcl
- IoT Core MQTT 3.1.1 compatibility and WSS support: https://cloud.baidu.com/doc/IoTCore/s/ikahmphms
...
IoT Core MQTT application permission binds a BCE IAM AK/SK to one instance. The server derives the exact documented MQTT 3.1.1 username `bceiam@{iotCoreId}|{AK}|{timestamp_ms}|SHA256` and password through the two HMAC-SHA256-HEX steps, then connects only to `wss://<iotCoreId>.iot.gz.baidubce.com/mqtt` on 443 with WebSocket subprotocol `mqtt`. The official example vector is enforced by a hermetic test. The documented application-permission format has no BCE STS session-token field, so the adapter fails closed when one is configured. It never accepts or exposes the derived MQTT username/password.

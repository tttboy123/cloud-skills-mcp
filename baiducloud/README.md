...
本目录把 BCE 资源意图路由到统一 `cloud-skills-mcp` server 的 `baiducloud_api_discover`、`baiducloud_api_read` 和 `baiducloud_api_mutate`。底层对官方 `baidubce.com` HTTPS API 实现 `bce-auth-v1` 签名。

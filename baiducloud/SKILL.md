---
name: baidu-cloud
description: Operate or inspect any Baidu AI Cloud BCE resource through cloud-skills-mcp signed HTTPS, CCR Enterprise/Personal Docker or OCI Registry HTTP, and the guarded RTC AI Agent WebSocket lifecycle. Use for Baidu Cloud, BCE, BCC, BOS, VPC, RDS, IAM, CDN, CCE, CCR, container images, RTC AI Agent, or documented baidubce.com and bcebos.com APIs.
---

# Baidu AI Cloud

Use the universal BCE REST gateway. It implements the official `bce-auth-v1` HMAC-SHA256 signing contract and restricts requests to validated `baidubce.com` service endpoints plus official BOS `bcebos.com` HTTPS endpoints. CCR uses `ccr-registry`: the server obtains only one-hour Registry credentials from fixed BCE-signed control requests, performs the same-origin Harbor Bearer exchange, and never executes Docker or exposes a password/token. RTC AI Agent uses a separate guarded `rtc-aiagent-ws` plan: the server signs create/stop through BCE v1, keeps the returned instance token private, activates the operator-held product license internally, and publishes only sanitized bounded WSS events.
...
Read [references/official-docs.md](references/official-docs.md) for v1/v2 signing, IAM/STS, BOS, CCR Registry, and API-center references.

...
func newInvokeTool(name string, mutating bool) mcp.Tool {
	...
	options := []mcp.ToolOption{
		...
		mcp.WithAny("body", mcp.Description("Optional JSON-compatible REST request body. Google Cloud grpc protobuf-json accepts one ProtoJSON object for unary methods or a finite object array for client streams; AWS sigv4-ws accepts a finite text/JSON/binary message plan; Google Cloud vertex-live-ws accepts a bounded setup-first Live API message plan; connect-health-ws requires the Medical Scribe configuration event before body_file audio; Azure realtime-ws accepts one event object or an event array, webpubsub-ws accepts a bounded client-token/message plan, and webpubsub-mqtt-ws accepts a finite MQTT read or bidirectional client plan; AWS transcribe-ws accepts an optional ConfigurationEvent before body_file audio; AWS iot-mqtt-ws, appsync-event-ws, and appsync-graphql-ws accept finite subscription plans; Alibaba nls-ws accepts guarded start payloads or text segments; Alibaba nls-rest TTS POST accepts documented JSON fields; Baidu rtc-aiagent-ws accepts a credential-free instance/config/text/termination plan while all tokens and license material remain internal.")),
		mcp.WithString("body_file", mcp.Description("Optional local HTTP body or WebSocket stream file. AWS connect-health-ws and transcribe-ws stream finite audio; Azure realtime-ws accepts one JSON event per line; Google Cloud grpc requires standard 5-byte-framed, uncompressed protobuf messages; Alibaba NLS recognition reads finite audio; Baidu RTC accepts complete 640-byte raw16k frames paced at 20 ms. The resolved regular file must be under CLOUD_SKILLS_ALLOWED_FILE_ROOTS. Only explicit AWS Connect Health, AWS Transcribe, Alibaba nls-ws recognition, and Baidu rtc-aiagent-ws can combine it with body; Alibaba nls-rest ASR uses the file alone.")),

...
func newInvokeTool(name string, mutating bool) mcp.Tool {
	...
	options := []mcp.ToolOption{
		...
		mcp.WithString("auth_scheme", mcp.Description("Optional provider HTTP authentication scheme: sigv4, sigv4a, sigv4-ws, connect-health-ws, transcribe-ws, iot-mqtt-ws, kinesisvideo-signaling-ws, appsync-event-ws, appsync-graphql-ws, acr, realtime-ws, voice-live-ws, webpubsub-ws, webpubsub-mqtt-ws, eventgrid-mqtt-ws, servicebus-amqp-ws, eventhubs-amqp-ws, firebase-sse, grpc, vertex-live-ws, acs3, rpc, roa, datahub, opensearch, odps, odps4, fc, fc3, fc-custom, oss, oss4, sls, sls4, mns, mq, ots, ots4, nls-rest, nls-ws, tc3, tc1, tc1-sha256, qcloud, qcloud-sha256, asr-ws, virtual-number-ws, soe-ws, speech-translate-ws, voice-convert-ws, mps-ws, mps-tts-ws, tts-ws, tts-stream-ws, podcast-ws, cos, cls, rtc-aiagent-ws, or the provider default.")),
...
func invocationFromRequest(provider Provider, mode InvocationMode, request mcp.CallToolRequest) (Invocation, error) {
	...
	invocation := Invocation{
		...
		Subscription: request.GetString("subscription", ""), Audience: request.GetString("audience", ""), ACRScope: request.GetString("acr_scope", ""), ACRSourceScope: request.GetString("acr_source_scope", ""), AuthScheme: request.GetString("auth_scheme", ""), AuthVersion: request.GetString("auth_version", ""), APIVersion: request.GetString("api_version", ""), PayloadMode: request.GetString("payload_mode", ""), ChecksumAlgorithm: request.GetString("checksum_algorithm", ""), Method: request.GetString("method", ""),
...
func auditEvent(runtime Runtime, invocation Invocation, sensitive bool, outcome, requestID string) AuditEvent {
	return AuditEvent{
		...
		Subscription: invocation.Subscription, ACRScope: invocation.ACRScope, ACRSourceScope: invocation.ACRSourceScope, AuthScheme: invocation.AuthScheme, AuthVersion: invocation.AuthVersion, APIVersion: invocation.APIVersion, PayloadMode: invocation.PayloadMode, ChecksumAlgorithm: invocation.ChecksumAlgorithm, Sensitive: sensitive, Outcome: outcome, RequestID: requestID,
	}
}

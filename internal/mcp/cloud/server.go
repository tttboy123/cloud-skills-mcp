...
func newInvokeTool(name string, mutating bool) mcp.Tool {
	...
	options := []mcp.ToolOption{
		...
		mcp.WithString("auth_scheme", mcp.Description("Optional provider HTTP authentication scheme: sigv4, sigv4a, transcribe-ws, iot-mqtt-ws, realtime-ws, grpc, acs3, rpc, roa, datahub, opensearch, odps, odps4, fc, fc3, fc-custom, oss, oss4, sls, sls4, mns, ots, ots4, nls-rest, nls-ws, tc3, tc1, tc1-sha256, qcloud, qcloud-sha256, asr-ws, virtual-number-ws, soe-ws, speech-translate-ws, voice-convert-ws, mps-ws, mps-tts-ws, tts-ws, tts-stream-ws, podcast-ws, cos, or the provider default.")),
		...
		mcp.WithAny("body", mcp.Description("Optional JSON-compatible REST request body. Azure realtime-ws accepts one event object or an event array; AWS transcribe-ws accepts an optional ConfigurationEvent before body_file audio; AWS iot-mqtt-ws accepts a finite subscription plan; Alibaba nls-ws accepts guarded start payloads or text segments; Alibaba nls-rest TTS POST accepts documented JSON fields.")),
		...
		mcp.WithString("response_file", mcp.Description("Optional new local file for a successful HTTP body, raw gRPC response, WebSocket NDJSON events, or synthesized audio; required by AWS iot-mqtt-ws, Azure realtime-ws, Google Cloud grpc, Alibaba nls-ws synthesis, and Alibaba nls-rest TTS. The target must be under CLOUD_SKILLS_ALLOWED_FILE_ROOTS and is never overwritten.")),

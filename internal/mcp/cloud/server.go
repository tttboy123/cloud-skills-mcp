...
func newInvokeTool(name string, mutating bool) mcp.Tool {
	...
	options := []mcp.ToolOption{
		...
		mcp.WithString("registry_instance_id", mcp.Description("Alibaba ACR or Tencent TCR Enterprise Edition instance ID used only for an internal official temporary-credential request; never a Registry credential.")),
...
func invocationFromRequest(provider Provider, mode InvocationMode, request mcp.CallToolRequest) (Invocation, error) {
	...
	invocation := Invocation{
		...
		Subscription: request.GetString("subscription", ""), Audience: request.GetString("audience", ""), RegistryInstanceID: request.GetString("registry_instance_id", ""), ACRScope: request.GetString("acr_scope", ""), ACRSourceScope: request.GetString("acr_source_scope", ""), AuthScheme: request.GetString("auth_scheme", ""), AuthVersion: request.GetString("auth_version", ""), APIVersion: request.GetString("api_version", ""), PayloadMode: request.GetString("payload_mode", ""), ChecksumAlgorithm: request.GetString("checksum_algorithm", ""), Method: request.GetString("method", ""),
...
func auditEvent(runtime Runtime, invocation Invocation, sensitive bool, outcome, requestID string) AuditEvent {
	return AuditEvent{
		...
		Subscription: invocation.Subscription, RegistryInstanceID: invocation.RegistryInstanceID, ACRScope: invocation.ACRScope, ACRSourceScope: invocation.ACRSourceScope, AuthScheme: invocation.AuthScheme, AuthVersion: invocation.AuthVersion, APIVersion: invocation.APIVersion, PayloadMode: invocation.PayloadMode, ChecksumAlgorithm: invocation.ChecksumAlgorithm, Sensitive: sensitive, Outcome: outcome, RequestID: requestID,
	}
}

...
func newInvokeTool(name string, mutating bool) mcp.Tool {
	...
	options := []mcp.ToolOption{
		...
		mcp.WithString("auth_scheme", mcp.Description("Optional provider HTTP authentication scheme: sigv4, sigv4a, sigv4-ws, connect-health-ws, transcribe-ws, iot-mqtt-ws, kinesisvideo-signaling-ws, appsync-event-ws, appsync-graphql-ws, realtime-ws, webpubsub-ws, webpubsub-mqtt-ws, grpc, vertex-live-ws, acs3, rpc, roa, datahub, opensearch, odps, odps4, fc, fc3, fc-custom, oss, oss4, sls, sls4, mns, ots, ots4, nls-rest, nls-ws, tc3, tc1, tc1-sha256, qcloud, qcloud-sha256, asr-ws, virtual-number-ws, soe-ws, speech-translate-ws, voice-convert-ws, mps-ws, mps-tts-ws, tts-ws, tts-stream-ws, podcast-ws, cos, rtc-aiagent-ws, or the provider default.")),
		...
		mcp.WithAny("body", mcp.Description("Optional JSON-compatible REST request body. Google Cloud grpc protobuf-json accepts one ProtoJSON object for unary methods or a finite object array for client streams; AWS sigv4-ws accepts a finite text/JSON/binary message plan; Google Cloud vertex-live-ws accepts a bounded setup-first Live API message plan; connect-health-ws requires the Medical Scribe configuration event before body_file audio; Azure realtime-ws accepts one event object or an event array, webpubsub-ws accepts a bounded client-token/message plan, and webpubsub-mqtt-ws accepts a finite MQTT read or bidirectional client plan; AWS transcribe-ws accepts an optional ConfigurationEvent before body_file audio; AWS iot-mqtt-ws accepts finite MQTT 3.1.1/5.0 subscription plans, kinesisvideo-signaling-ws accepts a bounded WebRTC signaling plan, and appsync-event-ws/appsync-graphql-ws accept finite subscription plans; Alibaba nls-ws accepts guarded start payloads or text segments; Alibaba nls-rest TTS POST accepts documented JSON fields; Baidu rtc-aiagent-ws accepts a credential-free instance/config/termination/audio-codec plan, validated pre/post-audio commands, optional image mode, and bounded Function Call result templates whose session IDs are correlated internally, while all tokens and license material remain internal.")),
		mcp.WithString("body_file", mcp.Description("Optional local HTTP body or WebSocket stream file. AWS connect-health-ws and transcribe-ws stream finite audio; Azure realtime-ws accepts one JSON event per line; Google Cloud grpc requires standard 5-byte-framed, uncompressed protobuf messages; Alibaba NLS recognition reads finite audio; Baidu RTC accepts raw, raw16k, PCMA, PCMU, G.722, or Opus packet streams, with variable Opus packet boundaries declared in the body plan. The resolved regular file must be under CLOUD_SKILLS_ALLOWED_FILE_ROOTS. Only explicit AWS Connect Health, AWS Transcribe, Alibaba nls-ws recognition, and Baidu rtc-aiagent-ws can combine it with body; Alibaba nls-rest ASR uses the file alone.")),

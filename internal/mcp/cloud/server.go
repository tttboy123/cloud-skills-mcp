...
func newInvokeTool(name string, mutating bool) mcp.Tool {
	...
	options := []mcp.ToolOption{
		...
		mcp.WithString("auth_scheme", mcp.Description("Optional provider HTTP authentication scheme: sigv4, sigv4a, transcribe-ws, grpc, acs3, rpc, roa, datahub, opensearch, odps, odps4, fc, fc3, fc-custom, oss, oss4, sls, sls4, mns, ots, ots4, tc3, tc1, tc1-sha256, qcloud, qcloud-sha256, asr-ws, virtual-number-ws, soe-ws, speech-translate-ws, voice-convert-ws, mps-ws, mps-tts-ws, tts-ws, tts-stream-ws, podcast-ws, cos, or the provider default.")),
		...
		mcp.WithAny("body", mcp.Description("Optional JSON-compatible REST request body; with AWS transcribe-ws it is an optional ConfigurationEvent sent before body_file audio.")),
		mcp.WithString("body_file", mcp.Description("Optional local HTTP body or WebSocket binary-stream file. Google Cloud grpc requires standard 5-byte-framed, uncompressed protobuf messages. The resolved regular file must be under CLOUD_SKILLS_ALLOWED_FILE_ROOTS and cannot be combined with body.")),
		mcp.WithString("response_file", mcp.Description("Optional new local file for a successful HTTP body or WebSocket NDJSON messages; required by Google Cloud grpc for raw framed protobuf responses. The target must be under CLOUD_SKILLS_ALLOWED_FILE_ROOTS and is never overwritten.")),
		mcp.WithNumber("stream_chunk_bytes", mcp.Description("Optional WebSocket binary message size. AWS Transcribe and Tencent audio streams derive protocol-specific defaults when omitted."), mcp.Min(1), mcp.Max(maxRequestFileBytes)),
		mcp.WithNumber("stream_interval_ms", mcp.Description("Optional streaming message pacing interval in milliseconds; Google Cloud grpc paces framed protobuf messages, AWS Transcribe defaults to 100 ms, Tencent ASR to 200 ms, and Tencent MPS PCM to 40 ms."), mcp.Min(1), mcp.Max(5000)),

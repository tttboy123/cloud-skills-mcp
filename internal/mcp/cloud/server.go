...
func newInvokeTool(name string, mutating bool) mcp.Tool {
	...
	options := []mcp.ToolOption{
		...
		mcp.WithString("auth_scheme", mcp.Description("Optional provider HTTP authentication scheme: sigv4, sigv4a, transcribe-ws, realtime-ws, grpc, acs3, rpc, roa, datahub, opensearch, odps, odps4, fc, fc3, fc-custom, oss, oss4, sls, sls4, mns, ots, ots4, nls-ws, tc3, tc1, tc1-sha256, qcloud, qcloud-sha256, asr-ws, virtual-number-ws, soe-ws, speech-translate-ws, voice-convert-ws, mps-ws, mps-tts-ws, tts-ws, tts-stream-ws, podcast-ws, cos, or the provider default.")),
		...
		mcp.WithAny("body", mcp.Description("Optional JSON-compatible REST request body. Azure realtime-ws accepts one event object or an event array; AWS transcribe-ws accepts an optional ConfigurationEvent before body_file audio; Alibaba nls-ws accepts guarded start payloads or text segments.")),
		mcp.WithString("body_file", mcp.Description("Optional local HTTP body or WebSocket stream file. Azure realtime-ws accepts one JSON event per line; Google Cloud grpc requires standard 5-byte-framed, uncompressed protobuf messages; Alibaba NLS recognition reads finite audio. The resolved regular file must be under CLOUD_SKILLS_ALLOWED_FILE_ROOTS. Only the explicit AWS Transcribe and Alibaba NLS recognition protocols can combine it with body.")),
		mcp.WithString("response_file", mcp.Description("Optional new local file for a successful HTTP body, raw gRPC response, WebSocket NDJSON events, or synthesized audio; required by Azure realtime-ws, Google Cloud grpc, and Alibaba nls-ws. The target must be under CLOUD_SKILLS_ALLOWED_FILE_ROOTS and is never overwritten.")),

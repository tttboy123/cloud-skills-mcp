...
func newInvokeTool(name string, mutating bool) mcp.Tool {
	...
	options := []mcp.ToolOption{
		...
		mcp.WithString("auth_scheme", mcp.Description("Optional provider HTTP authentication scheme: sigv4, sigv4a, ecr (private or public Docker/OCI Registry with internal IAM token retrieval), sigv4-ws, connect-health-ws, transcribe-ws, iot-mqtt-ws, kinesisvideo-signaling-ws, appsync-event-ws, appsync-graphql-ws, ivs-chat-ws (Amazon IVS Chat with an internal one-time IAM token), lex-v2-conversation (Amazon Lex V2 StartConversation with internal SigV4 stream signing), chime-messaging-ws (Amazon Chime SDK Messaging WebSocket with an internal SigV4 session endpoint and presigned connect), connect-chat-ws (Amazon Connect chat participant WebSocket with internal SigV4 StartChatContact and bearer CreateParticipantConnection), acr, realtime-ws, voice-live-ws, openai-chat-stream (Azure OpenAI Chat Completions text/event-stream with an internal Entra Bearer token), openai-responses-stream (Azure OpenAI Responses API text/event-stream with an internal Entra Bearer token), webpubsub-ws, signalr-ws (Azure SignalR Service serverless WSS with an internal Entra-backed client token), webpubsub-mqtt-ws, eventgrid-mqtt-ws, servicebus-amqp-ws, eventhubs-amqp-ws, artifact-registry (Google OCI Registry with internal ADC Bearer), firebase-sse, grpc, vertex-live-ws, acs3, rpc, roa, datahub, opensearch, odps, odps4, fc, fc3, fc-custom, oss, oss4, sls, sls4, mns, mq, acr-registry (Alibaba Enterprise Registry with internal RAM token exchange), ots, ots4, nls-rest, nls-ws, tc3, tc1, tc1-sha256, qcloud, qcloud-sha256, asr-ws, virtual-number-ws, soe-ws, speech-translate-ws, voice-convert-ws, mps-ws, mps-tts-ws, tts-ws, tts-stream-ws, podcast-ws, cos, cls, tcr-registry (Tencent Enterprise Registry with internal CAM temporary credential), ccr-registry (Baidu Enterprise or Personal Registry with internal BCE temporary credential), iotcore-http-pub (Baidu IoT Core HTTPS publish with an internal short-lived application token), iotcore-mqtt-ws (Baidu IoT Core MQTT 3.1.1/5.0 WSS with internal IAM application-permission signature), rtc-aiagent-ws, or the provider default.")),
		...
		mcp.WithString("api_version", mcp.Description("Provider API version used by Alibaba ACS3/RPC/ROA/DataHub/SLS/MNS/MQ/OTS, Tencent TC3 common parameters or headers, and the Azure OpenAI openai-chat-stream/openai-responses-stream queries.")),
		...
		mcp.WithAny("body", mcp.Description("Optional JSON-compatible REST request body. Google Cloud firebase-sse uses a finite max_events/timeout_seconds observation plan; grpc protobuf-json accepts one ProtoJSON object for unary methods or a finite object array for client streams; AWS sigv4-ws accepts a finite text/JSON/binary message plan; Google Cloud vertex-live-ws accepts a bounded setup-first Live API message plan; connect-health-ws requires the Medical Scribe configuration event before body_file audio; AWS chime-messaging-ws accepts a bounded SubscribeMessages plan with an official AppInstanceUser ARN, unique session ID, optional prefetch-on=connect and bounded connect expiry; AWS connect-chat-ws accepts a bounded instance/contact-flow/display-name plan with optional initial_message and attributes, plus max_events and timeout_seconds, and internally starts a chat contact and observes aws/chat events; Azure realtime-ws and voice-live-ws accept one event object or an event array, openai-chat-stream accepts a finite model/messages/max_tokens/temperature plan with max_events and timeout_seconds while stream:true is injected internally, openai-responses-stream accepts a finite model/input/max_output_tokens/temperature plan with max_events and timeout_seconds while stream:true and store:false are injected internally, webpubsub-ws accepts a bounded client-token/message plan, signalr-ws accepts a bounded Subscribe observation plan or a finite Invoke plan whose hub invocations carry bounded unique IDs, arguments, and response bounds, webpubsub-mqtt-ws or eventgrid-mqtt-ws accepts a finite MQTT read or bidirectional client plan, and servicebus-amqp-ws/eventhubs-amqp-ws accepts bounded broker operations over AMQP 1.0 WSS with internal Entra/CBS; AWS transcribe-ws accepts an optional ConfigurationEvent before body_file audio; AWS iot-mqtt-ws accepts bounded MQTT 3.1.1/5.0 subscribe/unsubscribe/publish, persistent-session, retained, supported property, and Will plans, kinesisvideo-signaling-ws accepts a bounded WebRTC signaling plan, and appsync-event-ws/appsync-graphql-ws accept finite subscription plans; Alibaba mq accepts a bounded RocketMQ 4.x publish or consume-and-internal-settlement plan whose receipt handles never leave the server; Alibaba nls-ws accepts guarded start payloads or text segments; Alibaba nls-rest TTS POST accepts documented JSON fields; Baidu iotcore-http-pub accepts a credential-free topic/QoS/base64 plan and derives its short-lived /auth token internally; Baidu iotcore-mqtt-ws accepts bounded credential-free MQTT 3.1.1/5.0 QoS 0/1/2 subscribe/unsubscribe/publish, MQTT 5 properties, and Will plans while deriving the IAM application username/password internally; Baidu rtc-aiagent-ws accepts a credential-free instance/config/termination/audio-codec plan, validated pre/post-audio commands, optional image mode, and bounded Function Call result templates whose session IDs are correlated internally, while all tokens and license material remain internal.")),
		mcp.WithString("body_file", mcp.Description("Optional local HTTP body or WebSocket stream file. AWS connect-health-ws and transcribe-ws stream finite audio; Azure realtime-ws and voice-live-ws accept one JSON event per line; Google Cloud grpc requires standard 5-byte-framed, uncompressed protobuf messages; Alibaba NLS recognition reads finite audio; Alibaba mq deliberately forbids body_file and builds bounded XML internally; Baidu IoT Core HTTP publish accepts one bounded binary payload while its topic/QoS remain in body; Baidu RTC accepts raw, raw16k, PCMA, PCMU, G.722, or Opus packet streams, with variable Opus packet boundaries declared in the body plan. The resolved regular file must be under CLOUD_SKILLS_ALLOWED_FILE_ROOTS. Only explicit AWS Connect Health, AWS Transcribe, Alibaba nls-ws recognition, Baidu iotcore-http-pub, and Baidu rtc-aiagent-ws can combine it with body; Alibaba nls-rest ASR uses the file alone.")),
		...
		mcp.WithString("response_file", mcp.Description("Optional new local file for a successful HTTP body, validated AWS EventStream response, Firebase SSE or gRPC NDJSON, WebSocket/AMQP NDJSON events, or synthesized audio; required by paced AWS aws-eventstream and by AWS sigv4-ws, connect-health-ws, kinesisvideo-signaling-ws, appsync-event-ws, appsync-graphql-ws, chime-messaging-ws, connect-chat-ws, and lex-v2-conversation, by AWS iot-mqtt-ws when messages are collected, Azure realtime-ws, voice-live-ws, openai-chat-stream, openai-responses-stream, webpubsub-ws, signalr-ws, webpubsub-mqtt-ws, eventgrid-mqtt-ws, servicebus-amqp-ws, and eventhubs-amqp-ws, Google Cloud firebase-sse, grpc, and vertex-live-ws, Alibaba mq, Alibaba nls-ws synthesis, Alibaba nls-rest TTS, Baidu iotcore-mqtt-ws subscriptions, and Baidu rtc-aiagent-ws. The target must be under CLOUD_SKILLS_ALLOWED_FILE_ROOTS and is never overwritten.")),

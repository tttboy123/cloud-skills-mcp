...
func newInvokeTool(name string, mutating bool) mcp.Tool {
	...
	options := []mcp.ToolOption{
		...
		mcp.WithString("response_file", mcp.Description("Optional new local file for a successful HTTP body, raw gRPC response, WebSocket NDJSON events, or synthesized audio; required by AWS sigv4-ws, connect-health-ws, iot-mqtt-ws, appsync-event-ws, and appsync-graphql-ws, Azure realtime-ws, Google Cloud grpc, Alibaba nls-ws synthesis, and Alibaba nls-rest TTS. The target must be under CLOUD_SKILLS_ALLOWED_FILE_ROOTS and is never overwritten.")),

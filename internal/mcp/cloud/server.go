...
func newInvokeTool(name string, mutating bool) mcp.Tool {
	...
	options := []mcp.ToolOption{
		...
		mcp.WithString("auth_scheme", mcp.Description("Optional provider HTTP authentication scheme: sigv4, sigv4a, sigv4-ws, connect-health-ws, transcribe-ws, iot-mqtt-ws, appsync-event-ws, appsync-graphql-ws, realtime-ws, grpc, acs3, rpc, roa, datahub, opensearch, odps, odps4, fc, fc3, fc-custom, oss, oss4, sls, sls4, mns, ots, ots4, nls-rest, nls-ws, tc3, tc1, tc1-sha256, qcloud, qcloud-sha256, asr-ws, virtual-number-ws, soe-ws, speech-translate-ws, voice-convert-ws, mps-ws, mps-tts-ws, tts-ws, tts-stream-ws, podcast-ws, cos, or the provider default.")),
		...
		mcp.WithString("response_file", mcp.Description("Optional new local file for a successful HTTP body, validated AWS EventStream response, raw gRPC response, WebSocket NDJSON events, or synthesized audio; required by paced AWS aws-eventstream and by AWS sigv4-ws, connect-health-ws, iot-mqtt-ws, appsync-event-ws, and appsync-graphql-ws, Azure realtime-ws, Google Cloud grpc, Alibaba nls-ws synthesis, and Alibaba nls-rest TTS. The target must be under CLOUD_SKILLS_ALLOWED_FILE_ROOTS and is never overwritten.")),
		...
		mcp.WithNumber("stream_interval_ms", mcp.Description("Optional streaming message pacing interval in milliseconds; AWS aws-eventstream paces consecutive pre-encoded frames and requires response_file, Azure realtime-ws paces JSON events, Google Cloud grpc paces framed protobuf messages, AWS Connect Health, AWS Transcribe, and Alibaba NLS recognition default to 100 ms, Tencent ASR to 200 ms, and Tencent MPS PCM to 40 ms."), mcp.Min(1), mcp.Max(5000)),

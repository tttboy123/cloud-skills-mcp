...
func newInvokeTool(name string, mutating bool) mcp.Tool {
	...
	options := []mcp.ToolOption{
		...
		mcp.WithString("auth_scheme", mcp.Description("Optional provider HTTP authentication scheme: sigv4, sigv4a, sigv4-ws, connect-health-ws, transcribe-ws, iot-mqtt-ws, appsync-event-ws, appsync-graphql-ws, realtime-ws, webpubsub-ws, webpubsub-mqtt-ws, grpc, vertex-live-ws, acs3, rpc, roa, datahub, opensearch, odps, odps4, fc, fc3, fc-custom, oss, oss4, sls, sls4, mns, ots, ots4, nls-rest, nls-ws, tc3, tc1, tc1-sha256, qcloud, qcloud-sha256, asr-ws, virtual-number-ws, soe-ws, speech-translate-ws, voice-convert-ws, mps-ws, mps-tts-ws, tts-ws, tts-stream-ws, podcast-ws, cos, or the provider default.")),
		...
		mcp.WithString("payload_mode", mcp.Description("Optional AWS payload protocol: aws-chunked for SigV4/SigV4a S3 streaming PutObject/UploadPart, aws-chunked-trailer for a server-computed signed checksum trailer, or aws-eventstream for bounded SigV4 CRC-valid frames over a finite or bidirectional HTTP/2 request.")),
		...
		mcp.WithAny("body", mcp.Description("Optional JSON-compatible REST request body. AWS sigv4-ws accepts a finite text/JSON/binary message plan; Google Cloud vertex-live-ws accepts a bounded setup-first Live API message plan; connect-health-ws requires the Medical Scribe configuration event before body_file audio; Azure realtime-ws accepts one event object or an event array, webpubsub-ws accepts a bounded client-token/message plan, and webpubsub-mqtt-ws accepts a finite MQTT read or bidirectional client plan; AWS transcribe-ws accepts an optional ConfigurationEvent before body_file audio; AWS iot-mqtt-ws, appsync-event-ws, and appsync-graphql-ws accept finite subscription plans; Alibaba nls-ws accepts guarded start payloads or text segments; Alibaba nls-rest TTS POST accepts documented JSON fields.")),
		mcp.WithString("body_file", mcp.Description("Optional local HTTP body or WebSocket stream file. AWS connect-health-ws and transcribe-ws stream finite audio; Azure realtime-ws accepts one JSON event per line; Google Cloud grpc requires standard 5-byte-framed, uncompressed protobuf messages; Alibaba NLS recognition reads finite audio. The resolved regular file must be under CLOUD_SKILLS_ALLOWED_FILE_ROOTS. Only explicit AWS Connect Health, AWS Transcribe, and Alibaba nls-ws recognition can combine it with body; Alibaba nls-rest ASR uses the file alone.")),
		mcp.WithString("response_file", mcp.Description("Optional new local file for a successful HTTP body, validated AWS EventStream response, raw gRPC response, WebSocket NDJSON events, or synthesized audio; required by paced AWS aws-eventstream and by AWS sigv4-ws, connect-health-ws, iot-mqtt-ws, appsync-event-ws, and appsync-graphql-ws, Azure realtime-ws, webpubsub-ws, and webpubsub-mqtt-ws, Google Cloud grpc and vertex-live-ws, Alibaba nls-ws synthesis, and Alibaba nls-rest TTS. The target must be under CLOUD_SKILLS_ALLOWED_FILE_ROOTS and is never overwritten.")),
		mcp.WithNumber("stream_chunk_bytes", mcp.Description("Optional WebSocket binary message size. AWS Connect Health, AWS Transcribe, Alibaba NLS recognition, and Tencent audio streams derive protocol-specific defaults when omitted."), mcp.Min(1), mcp.Max(maxRequestFileBytes)),
		mcp.WithNumber("stream_interval_ms", mcp.Description("Optional streaming message pacing interval in milliseconds; AWS aws-eventstream paces consecutive pre-encoded frames and requires response_file, Azure realtime-ws/webpubsub-ws and Google Cloud vertex-live-ws pace JSON events, Google Cloud grpc paces framed protobuf messages, AWS Connect Health, AWS Transcribe, and Alibaba NLS recognition default to 100 ms, Tencent ASR to 200 ms, and Tencent MPS PCM to 40 ms."), mcp.Min(1), mcp.Max(5000)),

...
func newInvokeTool(name string, mutating bool) mcp.Tool {
	...
	options := []mcp.ToolOption{
		...
		mcp.WithAny("body", mcp.Description("Optional JSON-compatible REST request body. AWS sigv4-ws accepts a finite text/JSON/binary message plan; Google Cloud vertex-live-ws accepts a bounded setup-first Live API message plan; connect-health-ws requires the Medical Scribe configuration event before body_file audio; Azure realtime-ws accepts one event object or an event array, webpubsub-ws accepts a bounded client-token/message plan, and webpubsub-mqtt-ws accepts a finite MQTT subscription plan; AWS transcribe-ws accepts an optional ConfigurationEvent before body_file audio; AWS iot-mqtt-ws, appsync-event-ws, and appsync-graphql-ws accept finite subscription plans; Alibaba nls-ws accepts guarded start payloads or text segments; Alibaba nls-rest TTS POST accepts documented JSON fields.")),

---
name: azure-cloud
description: Operate or inspect Azure resources through direct authenticated HTTPS, Azure OpenAI Realtime WSS, or Entra-backed Azure Web PubSub JSON WSS in cloud-skills-mcp. Use for Azure, ARM, Entra ID, Microsoft Graph, subscriptions, resource groups, Azure OpenAI Realtime, Web PubSub, or documented Azure APIs.
---
...
4. Azure Web PubSub `auth_scheme="webpubsub-ws"` always uses `azure_api_mutate(force=true)`: the server internally mints a five-minute client token through Entra and starts a paid stateful PubSub session.
...
- `auth_scheme="webpubsub-ws"`: use service `webpubsub`, operation `ClientConnect`, method `GET`, and the exact public-cloud `wss://<resource>.webpubsub.azure.com/client/hubs/<hub>` URL. `body` contains optional `protocol="json|json-reliable"` (default `json`), bounded `user_id`, documented `roles` and initial `groups`, 1–256 official JSON subprotocol `messages`, `max_messages`, and `timeout_seconds` (1–300). Reliable mode requires an `ackId` on every non-ping publisher message, negotiates `json.reliable.webpubsub.azure.v1`, acknowledges exact uint64 sequences, drops duplicates, retries recovery for at most one minute, and resends only unacknowledged publisher messages. `response_file` is required. Caller headers, query/recovery parameters, access tokens, client-token/reconnection-token responses, custom endpoints, and body files are forbidden.
- `auth_scheme="webpubsub-mqtt-ws"`: use the read tool with service `webpubsub`, operation `SubscribeMQTT`, method `GET`, and exact `wss://<resource>.webpubsub.azure.com/clients/mqtt/hubs/<hub>`. The finite MQTT 3.1.1 body requires an alphanumeric 1–128 byte `client_id`, 1–8 exact non-wildcard topic subscriptions at QoS 0/1, `keep_alive_seconds` (1–180), `max_messages`, and `timeout_seconds`. The gateway derives only the required join roles, mints `clientType=MQTT` internally, performs CONNACK/SUBACK/PUBACK and bounded keepalive, and writes Base64 payload NDJSON atomically. MQTT 5, publish, QoS 2, retained, shared, wildcard, Last Will, username/password, caller tokens, and persistent-session controls are not accepted by this entrypoint.

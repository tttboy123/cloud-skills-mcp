...
- `auth_scheme="webpubsub-ws"`: use service `webpubsub`, operation `ClientConnect`, method `GET`, and the exact public-cloud `wss://<resource>.webpubsub.azure.com/client/hubs/<hub>` URL. `body` contains optional bounded `user_id`, documented `roles` and initial `groups`, 1–256 official JSON subprotocol `messages`, `max_messages`, and `timeout_seconds` (1–300). `response_file` is required. Caller headers, query parameters, access tokens, client-token responses, custom endpoints, and body files are forbidden.

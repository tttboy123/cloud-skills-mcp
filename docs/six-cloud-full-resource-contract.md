...
| Tencent Cloud | Direct API 3.0 TC3 and v1 HmacSHA1/HmacSHA256 HTTPS, still-active qcloud API 2017 query/form HTTPS, COS data-plane signed HTTPS, and internally connected realtime ASR/MPS recognition plus MPS TTS signed WSS | SecretId/SecretKey or CAM/STS temporary credentials injected into the server environment; these WSS protocols use the documented long-lived SecretId/SecretKey tuple because they have no Token field |

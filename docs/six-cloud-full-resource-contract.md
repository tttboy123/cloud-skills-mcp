...
| Tencent Cloud | Direct API 3.0 TC3 and v1 HmacSHA1/HmacSHA256 HTTPS, still-active qcloud API 2017 query/form HTTPS, COS data-plane signed HTTPS, and internally connected realtime ASR/MPS signed WSS | SecretId/SecretKey or CAM/STS temporary credentials injected into the server environment; ASR/MPS WSS use the documented long-lived SecretId/SecretKey tuple because those protocols have no Token field |

...
| Protocol, installer, security and cross-platform build gates | `scripts/ci/protocol-smoke.sh`, `install-smoke.sh`, `build-release.sh`, `.github/workflows/ci.yml` | Current Azure Web PubSub MQTT 3.1.1/5.0 slice covers exact endpoint/schema validation, `clientType=MQTT` token containment, least-privilege join/send roles, subprotocol negotiation, QoS 0/1/2 bidirectional state machines, duplicate suppression, persistent-session and Last-Will encoding, MQTT 5 property/reason validation, negotiated broker flow control, keepalive, server/client disconnect, and atomic failure behavior | Module verification, formatting, vet, full race suite (80.1% total coverage), build, protocol/install smoke, shell syntax, six-Skill validation, Actionlint, govulncheck, and four-platform release builds passed locally; local ShellCheck unavailable; remote gates pending push |

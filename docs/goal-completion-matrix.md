...
| Protocol, installer, security and cross-platform build gates | `scripts/ci/protocol-smoke.sh`, `install-smoke.sh`, `build-release.sh`, `.github/workflows/ci.yml` | Current Azure Web PubSub MQTT 3.1.1 slice covers exact endpoint/schema validation, `clientType=MQTT` token containment, least-privilege topic roles, subprotocol negotiation, CONNACK/SUBACK/PUBACK, keepalive, and atomic failure behavior; preceding reliable JSON run [30779468405](https://github.com/tttboy123/cloud-skills-mcp/actions/runs/30779468405) passed macOS, Ubuntu, ShellCheck, security and release jobs | Module verification, formatting, vet, full race suite (80.1% total coverage), build, protocol/install smoke, shell syntax, six-Skill validation, Actionlint, govulncheck, and four-platform release builds passed locally; local ShellCheck unavailable; current remote gates pending push |

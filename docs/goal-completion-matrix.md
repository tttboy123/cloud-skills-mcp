...
| Protocol, installer, security and cross-platform build gates | `scripts/ci/protocol-smoke.sh`, `install-smoke.sh`, `build-release.sh`, `.github/workflows/ci.yml` | Current GCP raw gRPC slice: framed protobuf validation, ADC-authenticated HTTP/2 headers, pacing, trailer/status checks, atomic output and negative vectors plus local gates; the preceding AWS Transcribe slice passed all remote macOS, Ubuntu, ShellCheck, security and release jobs in GitHub Actions run [30768489384](https://github.com/tttboy123/cloud-skills-mcp/actions/runs/30768489384) | Current local and preceding remote gates passed; current remote gate runs on push |

...
| Protocol, installer, security and cross-platform build gates | `scripts/ci/protocol-smoke.sh`, `install-smoke.sh`, `build-release.sh`, `.github/workflows/ci.yml` | Current Azure OpenAI Realtime WSS slice: GA/preview target validation, Entra-only handshake, JSON/NDJSON event and terminal-count vectors plus local gates; the preceding GCP gRPC slice passed all remote macOS, Ubuntu, ShellCheck, security and release jobs in GitHub Actions run [30769154630](https://github.com/tttboy123/cloud-skills-mcp/actions/runs/30769154630) | Current local and preceding remote gates passed; current remote gate runs on push |

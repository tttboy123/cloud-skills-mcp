...
| All documented resource APIs addressable without per-resource Go handlers | Generic HTTP coverage exists for AWS SigV4/SigV4a, S3 SigV4/SigV4a `aws-chunked`, signed checksum trailers and finite SigV4 HTTP EventStream, Azure Entra, GCP ADC, Alibaba ACS3/RPC V2/ROA V2/OSS4/SLS v1/SLS v4/MNS/OTS v2/OTS v4, Tencent TC3/COS, and Baidu BCE v1/v2; remaining auth/transport families are enumerated in `api-protocol-coverage.md` | HTTP construction tests, AWS official SigV4a/S3 chunked/trailer key, framing, padding and signature vectors, official SDK StreamSigner and CRC64NVME vectors, official ACS3/RPC V2/ROA V2/SLS/OTS/TC3 vectors, MNS canonicalization vector, data-plane signer tests, and official API references | **Partial — interactive/non-REST, product-specific protocol audit and live vectors remain** |
...
| Protocol, installer, security and cross-platform build gates | `scripts/ci/protocol-smoke.sh`, `install-smoke.sh`, `build-release.sh`, `.github/workflows/ci.yml` | Current ROA V2 slice: local race coverage 82.4% overall and 82.5% in the core cloud package, vet, module verification, protocol/install smoke, six Skill validations, actionlint, vulnerability scan and four-target builds; the preceding RPC V2 slice passed macOS, Ubuntu, ShellCheck, security and release jobs in GitHub Actions run [30759820749](https://github.com/tttboy123/cloud-skills-mcp/actions/runs/30759820749) | Current local and preceding remote gates passed; current remote gate pending push |

...
| Protocol, installer, security and cross-platform build gates | `scripts/ci/protocol-smoke.sh`, `install-smoke.sh`, `build-release.sh`, `.github/workflows/ci.yml` | Current AWS HTTP/2 EventStream slice covers a real TLS HTTP/2 bidirectional transport, inter-frame pacing, concurrent request/response flow, chained signing, response length/CRC validation, ordinary input-only responses, and no-partial-output failures; preceding Connect Health run [30775454310](https://github.com/tttboy123/cloud-skills-mcp/actions/runs/30775454310) passed macOS, Ubuntu, ShellCheck, security and release jobs | Current local tests, race coverage (80.2%), vet, module verification, six-Skill validation, protocol/install smoke, shell syntax, Actionlint, govulncheck, and four-platform release builds passed; local ShellCheck unavailable; current remote gates pending push |

...
| Protocol, installer, security and cross-platform build gates | `scripts/ci/protocol-smoke.sh`, `install-smoke.sh`, `build-release.sh`, `.github/workflows/ci.yml` | Current AWS AppSync GraphQL WSS slice: distinct IAM `/graphql/connect` and subscription SigV4 authorization, dynamic auth-subprotocol containment, protocol acknowledgement/data/stop lifecycle, bounded collection, and atomic output vectors; preceding AWS AppSync Events run [30772935277](https://github.com/tttboy123/cloud-skills-mcp/actions/runs/30772935277) passed macOS, Ubuntu, ShellCheck, security and release jobs | Current local tests, race coverage (80.2%), vet, module verification, six-Skill validation, protocol/install smoke, shell syntax, Actionlint, govulncheck, and four-platform release builds passed; local ShellCheck unavailable; current remote gates pending push |

...
| Protocol, installer, security and cross-platform build gates | `scripts/ci/protocol-smoke.sh`, `install-smoke.sh`, `build-release.sh`, `.github/workflows/ci.yml` | Current Alibaba NLS WSS slice: public-boundary rejection, internal RPC V2 token signing/cache, five official task namespaces, start/stream/stop/terminal order, paced audio/text, token containment, atomic output and failure rollback vectors; preceding Azure OpenAI Realtime WSS remote run [30769785874](https://github.com/tttboy123/cloud-skills-mcp/actions/runs/30769785874) passed macOS, Ubuntu, ShellCheck, security and release jobs | Current focused local tests and preceding remote gates passed; full local and current remote gates run before/after push |

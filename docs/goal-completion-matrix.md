...
| Protocol, installer, security and cross-platform build gates | `scripts/ci/protocol-smoke.sh`, `install-smoke.sh`, `build-release.sh`, `.github/workflows/ci.yml` | Current AWS Connect Health WSS slice: exact endpoint/session validation, 60-second presigning, chained configuration/raw-audio/session-control EventStream frames, normal-close enforcement, mutation-only policy, and atomic transcript vectors; preceding generic AWS SigV4 WSS run [30774486400](https://github.com/tttboy123/cloud-skills-mcp/actions/runs/30774486400) passed macOS, Ubuntu, ShellCheck, security and release jobs | Current local tests, race coverage (80.1%), vet, module verification, six-Skill validation, protocol/install smoke, shell syntax, Actionlint, govulncheck, and four-platform release builds passed; local ShellCheck unavailable; current remote gates pending push |

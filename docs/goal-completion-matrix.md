...
| All documented resource APIs addressable without per-resource Go handlers | Generic HTTP coverage exists for AWS SigV4/SigV4a plus S3 SigV4 `aws-chunked`, Azure Entra, GCP ADC, Alibaba ACS3/OSS4, Tencent TC3/COS, and Baidu BCE v1/v2; remaining auth/transport families are enumerated in `api-protocol-coverage.md` | HTTP construction tests, AWS official SigV4a and S3 chunked vectors, official ACS3/TC3 vectors, data-plane signer tests, and official API references | **Partial — event-stream/non-REST, trailing headers, SigV4a streaming and product-specific protocol audit remains** |

...
| Protocol, installer, security and cross-platform build gates | `scripts/ci/protocol-smoke.sh`, `install-smoke.sh`, `build-release.sh`, `.github/workflows/ci.yml` | Current GCP Vertex Live WSS slice covers exact official endpoint/model validation, ADC Bearer containment, setup/setupComplete ordering, bounded client/server messages, mutation-only policy, and atomic failure behavior; preceding AWS HTTP/2 EventStream run [30776846685](https://github.com/tttboy123/cloud-skills-mcp/actions/runs/30776846685) passed macOS, Ubuntu, ShellCheck, security and release jobs | Current module verification, formatting, vet, full race suite (80.1% total coverage), build, protocol/install smoke, shell syntax, six-Skill validation, Actionlint, govulncheck, and four-platform release builds passed; local ShellCheck unavailable; current remote gates pending push |

...
| Protocol, installer, security and cross-platform build gates | `scripts/ci/protocol-smoke.sh`, `install-smoke.sh`, `build-release.sh`, `.github/workflows/ci.yml` | Current Azure Web PubSub WSS slice covers exact public endpoint and JSON-subprotocol validation, internal Entra-backed five-minute client-token minting, token containment, bounded messages, mutation-only policy, and atomic failure behavior; preceding GCP Vertex Live run [30777719559](https://github.com/tttboy123/cloud-skills-mcp/actions/runs/30777719559) passed macOS, Ubuntu, ShellCheck, security and release jobs | Current module verification, formatting, vet, full race suite (80.2% total coverage), build, protocol/install smoke, shell syntax, six-Skill validation, Actionlint, govulncheck, and four-platform release builds passed; local ShellCheck unavailable; current remote gates pending push |

...
| All documented resource APIs addressable without per-resource Go handlers | Generic HTTP coverage exists for AWS SigV4/SigV4a, S3 SigV4/SigV4a `aws-chunked`, signed checksum trailers and finite SigV4 HTTP EventStream, Azure Entra, GCP ADC, Alibaba ACS3/OSS4/SLS v1/SLS v4/MNS, Tencent TC3/COS, and Baidu BCE v1/v2; remaining auth/transport families are enumerated in `api-protocol-coverage.md` | HTTP construction tests, AWS official SigV4a/S3 chunked/trailer key, framing, padding and signature vectors, official SDK StreamSigner and CRC64NVME vectors, official ACS3/SLS/TC3 vectors, MNS canonicalization vector, data-plane signer tests, and official API references | **Partial — interactive/non-REST, product-specific protocol audit and live vectors remain** |

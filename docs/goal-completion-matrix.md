...
Latest verified slice: AWS Kinesis Video Streams now supports bounded
`ConnectAsMaster` and `ConnectAsViewer` WebRTC Signaling sessions over direct
WSS. The gateway implements the official 299-second `kinesisvideo` SigV4 query
algorithm, includes STS credentials only in the canonical query, enforces
role/ARN/recipient/correlation boundaries, internally Base64-encodes SDP/ICE
JSON, paces client frames, validates all documented asynchronous event types,
and atomically publishes decoded NDJSON without exposing the signed endpoint.
Implementation commit `b12206f1da49af59821ab2d6b661c36f13de991f` passed
remote macOS, Ubuntu, ShellCheck, Actionlint, govulncheck and four-platform
release verification in [CI run 30799880293](https://github.com/tttboy123/cloud-skills-mcp/actions/runs/30799880293).
...
Actionlint, govulncheck, four-platform archives and a five-second signaling
event fuzz run (126,413 executions) also passed.

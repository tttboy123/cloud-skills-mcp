...
| Baidu AI Cloud | BCE auth v1 and v2 signed HTTPS; AKSK/IAM-STS session token; CCR Enterprise and Personal Docker/OCI Registry V2 with fixed internal BCE v1 user/one-hour credential exchange, exact public/VPC/operator-custom endpoint validation, same-origin Bearer challenge binding, manifests/blobs/tags/referrers/uploads/mounts, redirect rejection, and internal temporary credential containment; RTC AI Agent BCE v1 create/private-instance-token WSS/stop lifecycle with exact official endpoints, operator-only license activation, all six documented upload codecs (`raw`, `raw16k`, `pcma`, `pcmu`, `g722`, `opus`), control/WSS codec agreement, 20–200 ms fixed-rate framing, variable-length Opus packet plans with official `ptime`/`plen`, strict credential-free parsing of the documented static break/text/TTS/device/GIS/player/ASR/prompt/variable/role/query/MCP/direct-control/meeting commands before or after audio, provider-event-correlated single-image upload with approved-root file validation and official 16 KiB/Base64 frame sequencing, current-format Function Call parsing with provider-session correlation to bounded credential-free `ok|error`/`post_function` templates, duplicate/unknown/call-limit rejection, serialized concurrent writes, bounded text/binary streaming, internal token containment, mutation-only policy, and atomic sanitized NDJSON | Product-specific legacy signatures or other long-lived transports outside BCE v1/v2, CCR, and RTC AI Agent; service-by-service live vectors |

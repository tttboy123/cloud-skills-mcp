...
- Amazon Connect chat realtime transport is credential-bound: the official
  participant flow uses `StartChatContact` then `CreateParticipantConnection`
  and authenticates the chat WebSocket with the returned participant
  `ConnectionToken` rather than the AWS identity chain. The Connect and

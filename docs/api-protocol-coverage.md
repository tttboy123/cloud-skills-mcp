...
|---|---|---|
| AWS | SigV4 header-signed HTTPS; SigV4 S3 `aws-chunked` streaming payload signing; finite SigV4 HTTP EventStream request signing and raw response-file streaming; SigV4a multi-region ECDSA header signing; AWS SDK default IAM/AKSK/STS chain | Interactive WebSocket sessions, signed trailing headers and SigV4a chunked payload signing; service-by-service live vectors |

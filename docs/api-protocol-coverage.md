...
| Baidu AI Cloud | BCE auth v1 and v2 signed HTTPS; AKSK/IAM-STS session token; RTC AI Agent BCE v1 create/private-instance-token WSS/stop lifecycle with exact official endpoints, operator-only license activation, all six documented upload codecs (`raw`, `raw16k`, `pcma`, `pcmu`, `g722`, `opus`), control/WSS codec agreement, 20–200 ms fixed-rate framing, variable-length Opus packet plans with official `ptime`/`plen`, bounded concurrent text/binary streaming, internal token containment, mutation-only policy, and atomic sanitized NDJSON | Product-specific legacy signatures or other long-lived transports outside BCE v1/v2 and RTC AI Agent; service-by-service live vectors |

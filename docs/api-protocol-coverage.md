...
| Tencent Cloud | TC3 API 3.0, API 3.0 v1 HmacSHA1/HmacSHA256 query/form, still-active legacy `*.api.qcloud.com/v2/index.php` HmacSHA1/HmacSHA256 query/form, COS REST signatures, realtime ASR HMAC-SHA1 WSS, MPS private-audio TC3 WSS recognition/translation with network-order framing, and MPS TC3 WSS streaming TTS with controlled text segments and atomic binary-audio output; AKSK/CAM credentials within each protocol's documented fields | Product-specific signatures outside TC3/v1/qcloud/COS/ASR/MPS recognition/TTS; other remaining long-lived streaming/WebSocket protocols; service-by-service live vectors |

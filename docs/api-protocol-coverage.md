...
- Amazon Chime SDK real-time meetings exchange WebRTC audio, video, and
  screen-share over the media plane after SigV4 control-plane meeting/attendee
  creation; the media frames are a non-resource transport and the Chime SDK
  control plane remains SigV4-addressable (the Chime SDK Messaging WebSocket
  implemented here is a separate messaging surface).
- AWS Elemental MediaPackage endpoints deliver Apple HLS, DASH-ISO, Microsoft
  Smooth Streaming, or CMAF media segments; endpoint delivery is the media
  plane, while the MediaPackage control plane remains SigV4-addressable.
- Amazon AppStream 2.0 application streaming sessions use the AppStream
  client with SAML 2.0 or certificate-based authentication, so the streaming
  transport is a credential-bound client surface; the AppStream 2.0 control
  plane remains SigV4-addressable.
...
- Google Cloud Cloud SQL instances speak the MySQL, PostgreSQL, and SQL
  ...
  generic ADC REST.
- Google Cloud Live Streaming API input ingest (SRT/RTMP) and output delivery
  (HLS/DASH via Media CDN or Cloud Storage) are the media plane; the API
  control plane remains addressable through generic ADC REST.
...
- Alibaba Cloud ApsaraDB RDS for MySQL and PostgreSQL speak the MySQL and
  PostgreSQL wire protocols with SSL encryption and instance username/password.
  Those are non-resource client line protocols; the RDS control plane remains
  ACS3/RPC addressable.
...
- Baidu AI Cloud Cloud Database RDS (MySQL, PostgreSQL, and SQL Server) speaks
  the corresponding database wire protocols; those are non-resource client
  line protocols, while the RDS control plane remains BCE-signed addressable.
